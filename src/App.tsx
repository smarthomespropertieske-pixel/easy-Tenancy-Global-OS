...
const AppDemo           = safeLazy(() => import('./routes/AppDemo'))
const GlobalDominance   = safeLazy(() => import('./routes/GlobalDominance'))
const PredictiveLifeOS  = safeLazy(() => import('./routes/PredictiveLifeOS'))
const RealEstateOS      = safeLazy(() => import('./routes/RealEstateOS'))
const PropertyDetail    = safeLazy(() => import('./routes/PropertyDetail'))
const SpatialStaging    = safeLazy(() => import('./components/SpatialStaging'))

// v4.4 — Roadmap preview shells (Feature 6 NOI · Feature F10 Investor)
const PortfolioPreview = safeLazy(() => import('./routes/PortfolioPreview'))
const InvestorPreview  = safeLazy(() => import('./routes/InvestorPreview'))
const NetZeroPage       = safeLazy(() => import('./routes/NetZeroPage'))
const AuthDashboard     = safeLazy(() => import('./routes/AuthDashboard'))
const GlobalPerformancePage = safeLazy(() => import('./routes/GlobalPerformancePage'))

// Security components (lazy — only needed when invoked)
const WebAuthnLogin   = safeLazy(() => import('./components/WebAuthnLogin'))
const TurnstileWidget = safeLazy(() => import('./components/TurnstileWidget'))
const FirebaseDiagnosticPage = safeLazy(() => import('./routes/FirebaseDiagnosticPage'))
const PropertyPublicView = safeLazy(() => import('./components/PropertyPublicView'))
...
function SpatialStagingPage() {
  ...
  return (
    <motion.div initial="initial" animate="enter" exit="exit" variants={PAGE_VARIANTS} style={{ minHeight: '100vh', background: '#0A0D14', paddingTop: 64 }}>
      <SEO title="Spatial Staging" description="Experience spatial staging powered by Novita AI." url="/spatial-staging" />
      <SovereignPageHeader
...
function SecurityDemoPage() {
  ...
  return (
    <motion.div initial="initial" animate="enter" exit="exit" variants={PAGE_VARIANTS} style={{ minHeight: '100vh', background: '#0A0D14', paddingTop: 64 }}>
      <SEO title="Security Architecture Demo" description="Global deployment security and FIDO2 authentication." url="/security-demo" />
      <SovereignPageHeader
      ...
      <SovereignMotionProvider>
          <Suspense fallback={<PageLoader />}>
            <Routes location={location}>
              ...
              {/* ── Net-Zero Carbon Telemetry Engine ───────────── */}
              <Route path="/netzero"
                element={
                  <Suspense fallback={<PageLoader label="Loading Net-Zero Engine…" />}>
                    <NetZeroPage />
                  </Suspense>
                }
              />
              ...
              <Route path="/global-performance"
                element={
                  <Suspense fallback={<PageLoader label="Loading Global Performance Dashboard…" />}>
              ...
              <Route path="/auth-dashboard"
                element={
                  <Suspense fallback={<PageLoader label="Loading Auth Dashboard…" />}>
                    <AuthDashboard />
                  </Suspense>
                }
              />
              ...
              {/* ── Public Property Mobile QR Passport View ────── */}
              <Route
                path="/public/property/:tenantId/:propId"
                element={
                  <Suspense fallback={<PageLoader label="Loading public property portal…" />}>
                    <PropertyPublicView />
                  </Suspense>
                }
              />
              <Route
                path="/public/property/:propId"
                element={
                  <Suspense fallback={<PageLoader label="Loading public property portal…" />}>
                    <PropertyPublicView />
                  </Suspense>
              ...
              />
              <Route path="/net-zero"
                element={<Navigate to="/netzero" replace />}
              />
              <Route path="/performance"
                element={<Navigate to="/global-performance" replace />}
              />
              <Route path="/security"
                element={<Navigate to="/security-demo" replace />}
              />
              <Route path="/firebase-diagnostic"
                element={<FirebaseDiagnosticPage />}
              />
              <Route path="/db-diagnostic"
                element={<Navigate to="/firebase-diagnostic" replace />}
              />
