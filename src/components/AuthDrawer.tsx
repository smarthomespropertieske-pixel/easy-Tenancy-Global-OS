// ════════════════════════════════════════════════════════════════════════
//  AuthDrawer.tsx — Slide-Out Sign-In & Profile Navigation Drawer v1.0
//  ─────────────────────────────────────────────────────────────────────
//  • Slide-out right panel with backdrop blur overlay
//  • Dual View Toggle: "Auth Mode" (Sign In / Passkeys) & "Account Settings"
//  • Seamless integration with Firebase Auth (Google OAuth, WebAuthn)
//  • Role & Persona switching directly inside account settings
//  • Developer API Bearer Token generator & Firestore session telemetry
// ════════════════════════════════════════════════════════════════════════
...
import { OAuthLoginGroup } from './OAuthLoginButton'
...
export default function AuthDrawer({
  ...
  const [tab, setTab] = useState<'auth' | 'account'>(initialTab)
  const [authError, setAuthError] = useState<string | null>(null)
  const [isSigningIn, setIsSigningIn] = useState(false)
  const [copiedToken, setCopiedToken] = useState(false)
  const [showPasskeyModal, setShowPasskeyModal] = useState(false)

  // Sync state with Firebase Auth if prop not strictly passed
  useEffect(() => {
  ...
  // Update tab based on authentication state on open
  useEffect(() => {
    if (isOpen) {
      setAuthError(null)
      if (activeUser) {
        setTab(initialTab === 'auth' && activeUser ? 'account' : initialTab)
      } else {
        setTab('auth')
      }
    }
      ...
      {isOpen && (
        <>
          ...
          <motion.aside
            ...
            role="dialog"
            aria-modal="true"
            aria-label="Profile & Authentication Navigation"
          >
            {/* Header Bar */}
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h2 className="text-sm font-semibold text-slate-100 flex items-center gap-1.5">
                    {activeUser ? activeUser.displayName || 'Account Dashboard' : 'Identity Gateway'}
                  </h2>
                  <p className="text-[11px] text-slate-400 font-mono">
                    easyTenancy.OS • Sovereign Auth
                  </p>
                </div>
              </div>
            ...
            {/* Navigation Tab Bar (Auth vs Account Settings) */}
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-6">
              {tab === 'auth' ? (
                ...
                <div className="space-y-5">
                  {/* Status Card if already logged in */}
                  {activeUser && (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="text-xs font-semibold text-slate-100 truncate">
                            {activeUser.displayName || 'Authenticated User'}
                          </p>
                  ...
                  <div className="space-y-3">
                    <p className="text-xs font-medium text-slate-400 uppercase tracking-wider font-mono">
                      {activeUser ? 'Switch or Re-Authenticate' : 'Sign In Options'}
                    </p>
                  ...
                  <div className="p-4 rounded-xl bg-slate-950/60 border border-white/10 space-y-3">
                    ...
                    <p className="text-[11px] text-slate-400 leading-relaxed">
                      Use Touch ID, Face ID, or your device PIN for instant biometric login without a password.
                    </p>
                    <button
                      onClick={() => setShowPasskeyModal(prev => !prev)}
                      className="w-full py-2 px-3 text-xs font-medium text-cyan-300 bg-cyan-950/40 hover:bg-cyan-900/50 border border-cyan-800/60 rounded-lg transition-colors flex items-center justify-center gap-2"
                    >
                      <Sparkles size={14} />
                      <span>{showPasskeyModal ? 'Hide Passkey Console' : 'Launch Passkey Biometric Auth'}</span>
                    </button>
                  ...
                  {/* Security / Bot Verification Guard */}
                ...
                <div className="space-y-6">
                  {/* User Profile Card */}
                  <div className="p-4 rounded-2xl bg-gradient-to-br from-slate-800/80 to-slate-900 border border-white/10 space-y-4">
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3">
                        {activeUser?.photoURL ? (
                          ...
                          <div className="w-12 h-12 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 flex items-center justify-center text-lg font-bold">
                            {activeUser?.displayName ? activeUser.displayName[0] : 'U'}
                          </div>
                        )}
                        <div>
                          ...
                          <p className="text-xs text-slate-400 font-mono">
                            {activeUser?.email || 'Not connected to Google'}
                          </p>
                          <div className="mt-1 flex items-center gap-1.5">
                            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-mono font-medium bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                              <Shield size={12} />
                              {activeUser ? 'Verified User' : 'Guest Tier'}
                            </span>
                          </div>
                        </div>
                      </div>
                  ...
                  {/* Active Role & Perspective Control */}
                  ...
                  <div className="p-4 rounded-xl bg-slate-950/80 border border-white/10 space-y-3">
                    ...
                    <p className="text-[11px] text-slate-400 leading-relaxed">
                      Use this bearer token to authenticate requests against EasyTenancy API endpoints like <code className="text-emerald-300 font-mono">/api/analytics</code> or <code className="text-emerald-300 font-mono">/api/waitlist</code>.
                    </p>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="px-3 py-2 bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-300 border border-emerald-500/30 rounded-lg text-xs font-medium transition-colors flex items-center gap-1.5 shrink-0"
                      >
                        {copiedToken ? (
                          <>
                            <Check size={14} className="text-emerald-400" />
                  ...
                  <div className="p-4 rounded-xl bg-slate-950/50 border border-white/5 space-y-2.5 text-xs">
                    ...
                    <div className="flex items-center justify-between py-1">
                      <span className="text-slate-400">Turnstile Protection</span>
                  ...
                  {activeUser ? (
                    <button
                      onClick={handleSignOut}
                      className="w-full py-2.5 px-4 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/20 text-rose-300 font-medium text-xs rounded-xl transition-colors flex items-center justify-center gap-2"
                    >
                      <LogOut size={16} />
                    ...
                    <button
                      onClick={() => setTab('auth')}
                      className="w-full py-2.5 px-4 bg-emerald-500/20 hover:bg-emerald-500/30 border border-emerald-500/30 text-emerald-300 font-semibold text-xs rounded-xl transition-colors flex items-center justify-center gap-2"
                    >
                      <Key size={16} />
                      <span>Sign In with Google / Passkey</span>
                    </button>
                  )}
                </div>
              )}
