...
import PropertyQRCodeModal from '../components/PropertyQRCodeModal'
import { GoogleSheetsWidget } from '../components/GoogleSheetsWidget'
import { PredictiveForecastWidget } from '../components/PredictiveForecastWidget'
import RecentActivityPanel from '../components/RecentActivityPanel'
import ThemeToggle from '../components/ThemeToggle'
import FloatingExportPDFButton from '../components/FloatingExportPDFButton'
...
// ── Property row ─────────────────────────────────────────────
function PropertyRow({ prop, currency, onNavigate, onOpenQr }: { prop: DemoProperty; currency: string; onNavigate: (propId: string, section: string) => void; onOpenQr?: (prop: DemoProperty) => void }) {
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          {onOpenQr && (
            <button
              onClick={e => { e.stopPropagation(); onOpenQr(prop) }}
              title="Generate QR Code Passport for Mobile Viewing"
              style={{
                background: 'rgba(56,189,248,0.15)', border: '1px solid rgba(56,189,248,0.3)',
                borderRadius: 6, padding: '4px 10px', fontSize: 11, color: '#38bdf8',
                fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s'
              }}
              onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.background = 'rgba(56,189,248,0.25)' }}
...
export default function AppDemo() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'properties' | 'files' | 'market' | 'actions' | 'ai'>('overview')
  const [selectedQrProperty, setSelectedQrProperty] = useState<DemoProperty | null>(null)
  ...
  // Track that we came from deep-link & sync URL tab/property search params
  useEffect(() => {
    const searchParams = new URLSearchParams(location.search)
    const tabParam = searchParams.get('tab')
    const propParam = searchParams.get('property')
    if (tabParam && ['overview', 'properties', 'files', 'market', 'actions', 'ai'].includes(tabParam)) {
  ...
  const handleExportMetricsCSV = () => {
    trackEvent('export_csv', { tenantId, type: 'metrics' })
    const metricsData = [
      { metric: 'Total Units', value: tenant.units },
      { metric: 'Monthly Rent', value: tenant.monthlyRent },
      { metric: 'Occupancy (%)', value: tenant.occupancy },
      { metric: 'NOI Margin (%)', value: tenant.noi },
      { metric: 'Arrears Rate (%)', value: tenant.arrears },
      { metric: 'Properties Count', value: tenant.properties.length }
    ]
    exportToCSV(
      `${tenant.name.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-metrics`,
      metricsData,
      [
  ...
  const handleExportCSV = () => {
    trackEvent('export_properties_csv', { tenantId, count: tenant.properties.length })
    exportToCSV(
      `${tenant.name.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-properties`,
      tenant.properties,
      [
        ...
        { key: 'lastCompliance', label: 'Last Compliance' },
  ...
  const handleExportPDF = async (action: 'download' | 'print' = 'download') => {
    trackEvent(`export_dashboard_pdf_${action}`, { tenantId })
    ...
    try {
      ...
      // Calculate PDF dimensions based on A4 size, preserving aspect ratio
      const pdf = new jsPDF({
        orientation: 'p',
        unit: 'px',
        format: 'a4'
      ...
      if (action === 'print') {
        ...
        if (typeof url === 'string') {
          ...
          // Fallback if bloburl returns an object URL instead
          const blobUrl = URL.createObjectURL(pdf.output('blob'))
          window.open(blobUrl, '_blank')
        }
      } else {
        pdf.save(`${tenant.name.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-investor-report.pdf`)
      }
    } catch (err) {
  ...
  return (
    <motion.div initial="initial" animate="enter" exit="exit" variants={PAGE_VARIANTS} style={{
      minHeight: '100vh',
      background: 'linear-gradient(135deg, #0a0f1e 0%, #0d1528 40%, #0a1525 100%)',
      paddingTop: 72,
      fontFamily: "'Inter', sans-serif"
    }}>
      <SEO title="App Demo" description="Pre-populated demo dashboard for property management." url="/app/demo" />
      ...
      <div id="dashboard-export-area" style={{ maxWidth: 1200, margin: '0 auto', padding: '24px 24px 80px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 12 }}>
          <div style={{ fontWeight: 600, fontSize: 13, color: '#64748b', textTransform: 'uppercase', letterSpacing: 0.8 }}>
            Performance Metrics
          </div>
          <button
            ...
            <motion.div key="overview" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 16 }}>
                <div>
                  ...
                  {tenant.properties.map(p => (
                    <PropertyRow key={p.id} prop={p} currency={tenant.currency} onNavigate={handlePropertyNav} onOpenQr={setSelectedQrProperty} />
                  ))}
                </div>
                <DemoFeedPanel orgId={tenantId} />
              </div>

              {/* Firestore Property Audit Recent Activity Panel */}
              <div style={{ marginTop: 16 }}>
                <RecentActivityPanel properties={tenant.properties} />
              </div>
              ...
              <div style={{ marginTop: 16 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 16, marginTop: 16 }}>
                  <PredictiveForecastWidget currentRevenue={tenant.monthlyRent} currency={tenant.currency} />
                  <GoogleSheetsWidget />
                </div>
              </div>
            </motion.div>
            ...
            <motion.div key="properties" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              ...
              {tenant.properties.map((p, i) => (
                ...
                >
                  ...
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    <button
                      onClick={() => setSelectedQrProperty(p)}
                      title="Generate QR Code Passport for Mobile Viewing"
                      style={{
                        padding: '6px 14px', borderRadius: 6, fontSize: 12, cursor: 'pointer',
                        border: '1px solid rgba(56,189,248,0.4)', background: 'rgba(56,189,248,0.12)',
                        color: '#38bdf8', fontWeight: 600, transition: 'all 0.2s',
                        display: 'inline-flex', alignItems: 'center', gap: 4
                      }}
                      onMouseEnter={e => { const b = e.currentTarget as HTMLButtonElement; b.style.background = 'rgba(56,189,248,0.22)' }}
                      onMouseLeave={e => { const b = e.currentTarget as HTMLButtonElement; b.style.background = 'rgba(56,189,248,0.12)' }}
                    >
                      📱 QR Code Passport
                    </button>
          ...
          {activeTab === 'files' && (
            <motion.div key="files" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              <div style={{ marginBottom: 16, fontWeight: 600, fontSize: 13, color: '#64748b', textTransform: 'uppercase', letterSpacing: 0.8 }}>
                Document Vault & Google Drive Sync — {tenant.name}
              </div>
              <FileLibrary tenantName={tenant.name} />
            </motion.div>
          )}

          {/* ── Market Performance tab ── */}
          {activeTab === 'market' && (
            <motion.div key="market" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              <MarketPerformance tenant={tenant} />
            </motion.div>
          )}
      ...
      <PropertyQRCodeModal
        property={selectedQrProperty}
        tenantId={tenantId}
        onClose={() => setSelectedQrProperty(null)}
      />

      <FloatingExportPDFButton
        tenant={tenant}
