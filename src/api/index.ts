...
api.post('/api/waitlist', async (c) => {
  try {
    const { email, persona, source, turnstileToken } = await c.req.json<{
      email: string; persona?: string; source?: string; turnstileToken?: string
    }>()
    ...
    if (turnstileToken) {
      const env = c.env as any
      const secretKey = env?.TURNSTILE_SECRET_KEY || (globalThis as any)?.process?.env?.TURNSTILE_SECRET_KEY || '1x000000000000000000000000000000AA'
      ...
      try {
        ...
        if (!outcome.success && !secretKey.startsWith('1x000000')) {
          return c.json({ ok: false, error: 'Bot verification failed. Please try again.' }, 400)
        }
      } catch (e) {
        console.warn('Turnstile verification check skipped due to network error:', e)
      }
    }
    ...
    if (env?.WAITLIST_KV) {
      await env.WAITLIST_KV.put(`wl:${email}`, JSON.stringify({
        email, persona, source, turnstileToken: !!turnstileToken, ts: Date.now()
      }), { expirationTtl: 60 * 60 * 24 * 365 })
    } else {
      console.log('[waitlist]', { email, persona, source, turnstileVerified: !!turnstileToken })
    }
...
api.post('/api/turnstile/verify', async (c) => {
  try {
    const { token, remoteip } = await c.req.json<{ token: string; remoteip?: string }>()
    if (!token) {
    ...
    if (remoteip || c.req.header('CF-Connecting-IP')) {
      formData.append('remoteip', remoteip || c.req.header('CF-Connecting-IP')!)
    }
    ...
    // Test secret or successful validation
    if (outcome.success || secretKey.startsWith('1x000000')) {
      return c.json({
        success: true,
        message: 'Turnstile verification succeeded',
        hostname: outcome.hostname ?? 'easytenancy.global',
        action: outcome.action ?? 'default',
        ts: outcome.challenge_ts ?? new Date().toISOString(),
      })
    } else {
      return c.json({
        success: false,
        error: 'Turnstile token verification failed',
        codes: outcome['error-codes'] ?? ['invalid-input-response'],
      }, 400)
    }
  } catch (err: any) {
    console.error('[Turnstile verification error]', err)
    return c.json({ success: false, error: 'Internal server error verifying token' }, 500)
  }
})
